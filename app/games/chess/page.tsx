...
// ✅ CDN Worker Stockfish (Next.js + Vercel için en sağlam yöntem)
function createStockfishWorker() {
  if (typeof window === "undefined") return null;

  // stockfish.js (wasm değil, pure js build) – Worker içinde çalışır
  ...
  const code = `
    ...
    function pick() {
      try { if (typeof self.Stockfish === "function") return self.Stockfish(); } catch (e) {}
      try { if (typeof self.stockfish === "function") return self.stockfish(); } catch (e) {}
      return null;
    }
    ...
    try {
      ...
      engine = pick();
      if (!engine) { self.postMessage("SF_INIT_FAILED"); return; }

      engine.onmessage = (e) => self.postMessage(e?.data ?? e);
...
export default function ChessPage() {
  ...
  useEffect(() => {
    ...
    w.onmessage = (e: any) => {
      ...
      if (msg === "SF_INIT_FAILED") {
        console.error("Stockfish init failed (CDN Worker).");
        setIsReady(false);
        return;
      }
  ...
  // =====================
  const game = useMemo(() => {
    const base = reviewIndex !== null && moveEvaluations[reviewIndex] ? moveEvaluations[reviewIndex].fenBefore : fen;
  ...
  function applyMoveFromEngine(from: Square, to: Square) {
    setFen((prev) => {
      ...
      try {
        ...
        playMoveSound();
        setMoveEvaluations((p) => [...p, { move: mv.san, score: lastScore.current, fenBefore, fenAfter }]);
  ...
  }, [mode, selectionStep, playerColor, isReady, reviewIndex, timeWinner, game, fen, difficulty]);

  // =====================
  // Hint
  // - online oynarken ipucu yok
  // - maç bitince incelemede ipucu var
  // =====================
  const getHint = (targetFen?: string) => {
    const overNow = game.isGameOver() || !!timeWinner;
    if (mode === "p2p" && !overNow) return; // ✅ online oyun içi ipucu yok
  ...
  // =====================
  // Timer (bot modda seçilen süre / online süresiz)
  // =====================
  useEffect(() => {
    let interval: any;

    if (selectionStep === "game" && initialTime && !game.isGameOver() && !timeWinner) {
  ...
  const totalScore = useMemo(() => {
    return moveEvaluations.reduce((sum, mv, i) => {
      const isWhite = i % 2 === 0;
      const isPlayerMove = (isWhite && playerColor === "w") || (!isWhite && playerColor === "b");
  ...
  // ✅ hooks bitti; şimdi return guard
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-sans text-white">
        <div className="bg-slate-900 p-10 rounded-[3rem] border border-white/5 w-full max-w-md shadow-2xl">
          <h1 className="text-3xl font-black italic uppercase mb-6 tracking-tighter text-center">TROPHY CHESS</h1>
          ...
          {!isReady && (
            <p className="mt-3 text-xs text-amber-300 text-center">Stockfish hazırlanıyor... (bot birazdan aktif olur)</p>
          )}
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-sans text-white">
        <div className="bg-slate-900 p-10 rounded-[3rem] border border-white/5 w-full max-w-md shadow-2xl">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-black uppercase italic tracking-widest text-indigo-400">Online Eşleşme</h2>
            <button
              onClick={() => {
                cleanupP2P();
                setMyCode("");
          ...
          <div className="mt-6">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">5 Haneli Kod ile Bağlan</label>
          ...
          {p2pStatus && (
            <div className="mt-3 text-xs text-slate-300 bg-slate-950/60 border border-white/5 rounded-2xl p-3">{p2pStatus}</div>
          )}
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-sans">
        <div className="bg-slate-900 p-12 rounded-[3rem] border border-white/5 text-center shadow-2xl w-full max-w-sm">
          <h1 className="text-4xl font-black text-white italic uppercase mb-10 tracking-tighter">X-CHESS</h1>
          ...
          {!isReady && <p className="mt-3 text-xs text-amber-300">Stockfish hazırlanıyor... (bot birazdan aktif)</p>}
        </div>
      </div>
    );
  }
  ...
  if (selectionStep === "time") {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-white font-sans">
        <div className="bg-slate-900 p-12 rounded-[3rem] border border-white/5 text-center shadow-2xl w-full max-w-sm">
          <h2 className="text-2xl font-black uppercase mb-8 italic tracking-widest text-indigo-400">Tempo Seç</h2>
            ...
            {opts.map((o) => (
              <button
                ...
                onClick={() => {
                  ...
                  // oyuncu siyahsa bot (beyaz) açılış yapsın
  ...
  const winnerText =
    timeWinner ||
    (game.isCheckmate() ? (game.turn() === "b" ? "BEYAZ KAZANDI" : "SİYAH KAZANDI") : "BERABERE");
        ...
        <div className="flex-1">
          <div className="bg-slate-900 rounded-[2.5rem] p-6 border border-white/5 shadow-2xl relative overflow-hidden">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-black italic tracking-tighter uppercase leading-none">X-CHESS</h1>
                  ...
                  {(["easy", "medium", "hard"] as const).map((lvl) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase transition-all ${
                        difficulty === lvl ? "bg-indigo-500 text-white" : "text-slate-500 hover:text-slate-300"
                      }`}
                    >
            ...
            <div className="mb-4 flex justify-between items-center p-4 rounded-2xl bg-slate-950 border border-white/5">
              ...
              {initialTime && playerColor && (
                <span className="text-xl font-mono font-black">{formatTime(playerColor === "w" ? blackTime : whiteTime)}</span>
              )}
            </div>
                ...
                displayFiles.map((f, fi) => {
                  ...
                  return (
                    <button
                      ...
                      } ${selected === square ? "bg-indigo-500/40 ring-4 ring-indigo-500 ring-inset" : "hover:bg-slate-600"}`}
                    >
            ...
            <div className="mt-4 flex justify-between items-center p-4 rounded-2xl bg-slate-950 border border-white/5">
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">SEN</span>
              {initialTime && playerColor && (
                <span className="text-xl font-mono font-black">{formatTime(playerColor === "w" ? whiteTime : blackTime)}</span>
              )}
            </div>
                ...
                <div className="bg-slate-900 p-10 rounded-[3rem] border-2 border-indigo-500/30 text-center shadow-2xl w-full max-w-sm">
                  <div className="relative mb-6 flex justify-center items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="text-[10px] font-black text-slate-500 uppercase">Toplam Skor</p>
                  ...
                  <h2 className="text-2xl font-black uppercase italic mb-4 tracking-tighter text-indigo-400">{winnerText}</h2>
                  ...
                  <button
                    onClick={() => {
                      if (mode === "p2p" && connRef.current?.open) connRef.current.send({ type: "reset" } as P2PMsg);
            ...
            <div className="mt-6 flex gap-4">
              {reviewIndex !== null ? (
                ...
                <>
                  {/* Bot modda ipucu var, online oyun içinde yok */}
                  {!isGameOver && mode !== "p2p" && (
                    <button
                      onClick={() => getHint()}
                      ...
                      className={`flex-1 py-4 text-white rounded-2xl font-black uppercase text-xs shadow-lg transition-all ${
                        hintCount >= 5 ? "bg-slate-800 opacity-50" : "bg-blue-600 hover:bg-blue-500"
                      }`}
                    >
          ...
          <div className="bg-slate-900 rounded-[2rem] p-6 border border-white/5 flex-1 flex flex-col shadow-xl min-h-[500px]">
            ...
            {mode === "p2p" && isGameOver && !analysisDone && (
              <div className="mb-4 text-[11px] text-slate-300 bg-slate-950/60 border border-white/5 rounded-2xl p-3">
                {postAnalyzing ? `Stockfish analiz ediyor... (${analysisProgress.i}/${analysisProgress.n})` : "Analiz başlatılıyor..."}
              </div>
            )}
                ...
                return (
                  <div
                    ...
                    className={`flex flex-col p-4 rounded-2xl transition-all border ${
                      ...
                      reviewIndex === i ? "border-blue-500 bg-blue-500/10" : "bg-slate-950/50 border-white/5"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] text-slate-600 font-black">{Math.floor(i / 2) + 1}.</span>
                        <span className={`font-bold ${isWhite ? "text-white" : "text-indigo-300"}`}>{evalData.move}</span>
                      </div>

                      {quality && <span className={`text-[9px] font-black uppercase ${quality.color}`}>{quality.label}</span>}
                    </div>

                    {quality && isPlayerMove && quality.puan > 0 && (
                      <div className="mt-2 text-[10px] font-black text-slate-500">+{quality.puan} Puan</div>
                    )}

                    {mode === "p2p" && isGameOver && !analysisDone && (
                      <div className="mt-2 text-[10px] font-black text-slate-500">Analiz bekleniyor...</div>
                    )}
                  </div>
                );
              })}
